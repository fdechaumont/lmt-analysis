...
class Animal:
    ...
    def filterDetectionToKeepOnlyHeadTailDetection(self):
        ...
        for key in sorted(self.detectionDictionary.keys()):
            ...
            if not a.isHeadAndTailDetected():
                ...
                nbRemoved += 1
        print(
            "Filtering head tail detection. number of detection removed:",
            nbRemoved,
        )
    ...
    def filterDetectionByArea(self, x1, y1, x2, y2):
        ...
        for key in sorted(self.detectionDictionary.keys()):
            ...
            x = (
                a.massX - self.parameters.cornerCoordinatesOpenFieldArea[0][0]
            ) * self.parameters.scaleFactor
            y = (
                a.massY - self.parameters.cornerCoordinatesOpenFieldArea[0][1]
            ) * self.parameters.scaleFactor
    ...
    def filterDetectionByDistanceToPoint(self, x1, y1, maxDistance):
        ...
        print(
            "Filtering by distance to point, number of detection removed:",
            nbRemoved,
        )
    ...
    def plotTrajectory(
        self, show=True, color="k", maskingEventTimeLine=None, title=""
    ):
    ...
    def getDistancePerBin(
        ...
        if filter_flickering or filter_stop:
            ...
            msg = "total"
        print(
            f"Compute {msg} distance between frames {minFrame} and {maxFrame}"
        )

        flicker_frames = {}
        if filter_flickering:
            flicker_frames = EventTimeLine(
                conn=self.conn, eventName="Flickering", idA=self.baseId
            ).getDictionary()

        stop_frames = {}
        if filter_stop:
            stop_frames = EventTimeLine(
                conn=self.conn, eventName="Stop", idA=self.baseId
            ).getDictionary()
    ...
    def _getSpeed(
        ...
        if speeds.size == 0:
            ...
            return (
                ...
                speeds.sum(),
                speeds.std(),
                speeds.std() / np.sqrt(speeds.size),
            )
    ...
    def getDistanceSpecZone(
        ...
        keyList = sorted(
            self.detectionDictionary.keys()
        )  # get the list of the frames where the animal has been detected
        ...
        for key in keyList:
            ...
            a = self.detectionDictionary.get(
                key
            )  # get the detection of the animal at the given frame
            b = self.detectionDictionary.get(
                key + 1
            )  # get the detection of the animal at the following frame
            ...
            distance += math.hypot(
                a.massX - b.massX, a.massY - b.massY
            )  # add the distance to the previously calculated distance
    ...
    def getDistanceTo(self, t, animalB):
        ...
        dist = math.hypot(
            self.detectionDictionary[t].massX
            - animalB.detectionDictionary[t].massX,
            self.detectionDictionary[t].massY
            - animalB.detectionDictionary[t].massY,
        )
    ...
    def getMeanDistanceToAnimalPerBin(
        self, binFrameSize, startFrame, endFrame, animalB
    ):
        if endFrame == None:
        ...
        while t < endFrame:
            print(t)
            distanceBin = self.getMeanDistanceTo(
                t, t + binFrameSize, animalB
            )  # computed in cm already
    ...
    def getDistanceNoseToPoint(self, t, xPoint, yPoint):
        ...
        if (
            ...
            distanceNoseToPoint = math.hypot(
                self.detectionDictionary[t].frontX - xPoint,
    ...
    def getThresholdMassHeight(self, tmin=0, tmax=None):
        ...
        self.eightDecileMassHeight = sorted(massHeightList)[
            math.ceil(decile) - 1
        ]
    ...
    def getThresholdFrontHeight(self, tmin=0, tmax=None):
        ...
        self.eightDecileFrontHeight = sorted(frontHeightList)[
            math.ceil(decile) - 1
        ]
    ...
    def getSap(self, tmin=0, tmax=None, xa=None, ya=None, xb=None, yb=None):
        ...
        for key in keyList:
            ...
            if (
                ...
                and speed < self.parameters.SPEED_THRESHOLD_LOW
...
class AnimalPool:
    ...
    def loadDetection(self, start=None, end=None, lightLoad=False):
        ...
        for animal in self.animalDictionary.keys():
            self.animalDictionary[animal].loadDetection(
                start=start, end=end, lightLoad=lightLoad
            )

    def filterDetectionByInstantSpeed(self, minSpeed, maxSpeed):
        for animal in self.animalDictionary.keys():
            self.animalDictionary[animal].filterDetectionByInstantSpeed(
                minSpeed, maxSpeed
            )
    ...
    def filterDetectionToKeepOnlyHeadTailDetection(self):
        for animal in self.animalDictionary.keys():
            self.animalDictionary[
                animal
            ].filterDetectionToKeepOnlyHeadTailDetection()
    ...
    def transformCoordinateCmToPixel(
        self, xCm, yCm, parameters=ParametersMouse()
    ):
        """
    ...
    def filterDetectionByDistanceToPoint(self, x, y, maxDistance):
        for animal in self.animalDictionary.keys():
            self.animalDictionary[animal].filterDetectionByDistanceToPoint(
                x, y, maxDistance
            )

    def filterDetectionByEventTimeLine(self, eventTimeLineVoc):
        for animal in self.animalDictionary.keys():
            self.animalDictionary[animal].filterDetectionByEventTimeLine(
                eventTimeLineVoc
            )
    ...
    def plotSensorData(
        ...
        # plot night
        nightTimeLine = EventTimeLine(
            self.conn, "night", None, None, None, None
        )
    ...
    def buildSensorData(self, file, show=False):
        ...
        self.plotSensorData(
            sensor="SOUND",
            saveFile=file + "_log_sound level.pdf",
            show=show,
            title=file,
        )
        self.plotSensorData(
    ...
    def getDetectionTable(self):
        ...
        * adds column to indicate detections in center region
        ...
        df["x_cm"] = (
            (df.x - self.parameters.cornerCoordinatesOpenFieldArea[0][0])
            / (
                self.parameters.cornerCoordinatesOpenFieldArea[1][0]
                - self.parameters.cornerCoordinatesOpenFieldArea[0][0]
            )
            * self.parameters.ARENA_SIZE
        )
        df["y_cm"] = (
            (df.y - self.parameters.cornerCoordinatesOpenFieldArea[1][1])
            / (
                self.parameters.cornerCoordinatesOpenFieldArea[2][1]
                - self.parameters.cornerCoordinatesOpenFieldArea[1][1]
            )
            * self.parameters.ARENA_SIZE
        )

        df[f"in_arena_center"] = (
            (df["x_cm"] > self.parameters.CENTER_MARGIN)
            & (df["y_cm"] > self.parameters.CENTER_MARGIN)
            & (
                df["x_cm"]
                < (self.parameters.ARENA_SIZE - self.parameters.CENTER_MARGIN)
            )
            & (
                df["y_cm"]
                < (self.parameters.ARENA_SIZE - self.parameters.CENTER_MARGIN)
