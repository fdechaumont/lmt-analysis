...
eventName = "FollowZone Isolated New2"  # FIX: CHECK IF that's not over propagated
...
def reBuildEvent( connection, file, tmin=None, tmax=None, pool = None ): 
    ...
    # filter detection by speed
    pool.filterDetectionByInstantSpeed( pool.getAnimalList()[0].parameters.SPEED_THRESHOLD_LOW , 1000 )
    ...
    for idAnimalA in pool.animalDictionnary:
        for idAnimalB in pool.animalDictionnary:
            if( idAnimalA == idAnimalB ):
                continue
            #eventName = "FollowZone Isolated New2" # FIX: remove                    
            print ( eventName )
    ...
    for idAnimalA in pool.animalDictionnary:
        for idAnimalB in pool.animalDictionnary:
            ...
            resultDic = {}
            # the question is: is A following B ?
