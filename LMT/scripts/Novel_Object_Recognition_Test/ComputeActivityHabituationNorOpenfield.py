...
from ComputeActivityHabituationNorTestRedCage import *
# from ComputeActivityHabituationNorTestRedCage import buildFigTrajectoryPerSetup,\
...
def plotVariablesHabituationNorBoxplotsPerTreatment(ax, sexList, treatmentList, data, val, unitDic, yMinDic, yMaxDic ):
    # reformate the data dictionary to get the correct format of data for plotting and statistical analyses
    dataVal = {}
    for sex in sexList:
        dataVal[sex] = {}
        for treatment in treatmentList:
            dataVal[sex][treatment] = []
            for rfid in data[val][sex][treatment].keys():
                dataVal[sex][treatment].append(data[val][sex][treatment][rfid])

    dic = {'treatment' : [], 'sex' : [], 'val': [], 'variable': []}
    for sex in sexList:
        for treatment in treatmentList:
            dic['variable'].extend( [val] * len(dataVal[sex][treatment]) )
            dic['val'].extend(dataVal[sex][treatment])
            dic['sex'].extend( [sex] * len(dataVal[sex][treatment]) )
            dic['treatment'].extend( [treatment] * len(dataVal[sex][treatment]) )

    df = pd.DataFrame.from_dict(dic)
    print(df)

    bp = sns.boxplot(data=df, x='sex', y='val', hue='treatment', hue_order=treatmentList, ax=ax, linewidth=0.5, showmeans=True,
                     meanprops={"marker": 'o',
    ...
    for patch in bp.artists:
        patch.set_facecolor(getColorGeno(treatmentList[k]))
        k += 1

    sns.stripplot(data=df, x='sex', y='val', hue='treatment', hue_order=treatmentList, jitter=True, color='black', s=5,
                  dodge=True, ax=ax)
    ...
    for sex in sexList:
        try:
            U, p = stats.mannwhitneyu(list(df['val'][(df['sex']==sex) & (df['treatment']==treatmentList[0])]), list(df['val'][(df['sex']==sex) & (df['treatment']==treatmentList[1])]))
...
if __name__ == '__main__':
    ...
    unitDic = {'totDistance': 'cm', 'centerDistance': 'cm', 'centerTime': 's', 's': 'occurrences', 'rearTotal Nb': 'occurrences', 'rearTotal Duration': 's',
    ...
    while True:
        ...
        question += "\n\t [2c] plot trajectories in the habituation phase per treatment?"
        ...
        question += "\n\t [3c] plot the distance and anxiety measures in the habituation phase per treatment?"
        ...
        if answer == '2':
            ...
            for file in files:
                ...
                data['sapTime'][sex][setup][rfid] = sap1 / 30
        ...
        if answer == '2c':
            print('Plot trajectory in the habituation phase per treatment')
            #distance travelled
            #space use
            #traj of center of mass
            files = getFilesToProcess() #upload files for the analysis
            ...
            for file in files:
                ...
                sap1 = len(animal.getSap(tmin=tminHab, tmax=tmaxHab, xa=128, xb=383, ya=80, yb=336))
                #fill the data dictionary with the computed data for each file:
                data['totDistance'][sex][treatment][rfid] = dt1
                data['distancePerBin'][sex][treatment][rfid] = dBin
                data['centerDistance'][sex][treatment][rfid] = d1
                data['centerTime'][sex][treatment][rfid] = t1 / 30
                data['nbSap'][sex][treatment][rfid] = sap1 / 30
                ...
                data['rearTotal Nb'][sex][treatment][rfid] = rearTotalTimeLine.getNbEvent()
                data['rearTotal Duration'][sex][treatment][rfid] = rearTotalTimeLine.getTotalLength() / 30
                data['rearCenter Nb'][sex][treatment][rfid] = rearCenterTimeLine.getNbEvent()
                data['rearCenter Duration'][sex][treatment][rfid] = rearCenterTimeLine.getTotalLength() / 30
                data['rearPeriphery Nb'][sex][treatment][rfid] = rearPeripheryTimeLine.getNbEvent()
                data['rearPeriphery Duration'][sex][treatment][rfid] = rearPeripheryTimeLine.getTotalLength() / 30
        ...
        if answer == '3c':
            ...
            for val in variableList:
                plotVariablesHabituationNorBoxplotsPerTreatment(ax=axes[col], sexList=sexList, treatmentList=treatmentList, data=data, val=val, unitDic=unitDic, yMinDic=yMinDic, yMaxDic=yMaxDic)
            ...
            plt.tight_layout()
            plt.show()  # display the plot
            fig.savefig('fig_values_hab_dyadic.pdf', dpi=200)
            fig.savefig('fig_values_hab_dyadic.jpg', dpi=200)
            print('Job done.')
            break
